...
class RealEstateGenerationDataset(Dataset):
    def __init__(self, args, mode, scenes, **kwargs):
        ...
        if isinstance(scenes, str):
            scenes = [scenes]
        # self.scene_path_list = glob.glob(os.path.join(self.folder_path, "frames", scenes))
        self.frame_path = os.path.join(self.folder_path, 'frames')

        all_rgb_files = []
        all_timestamps = []
        for scene in scenes:
            scene_path =  os.path.join(self.frame_path, scene)
    ...
    def __getitem__(self, idx):
        ...
        id_feat = np.random.choice(
            candidate_ids, size=min(self.num_source_views+1, len(candidate_ids)), replace=False
        )
        ...
        caption_rgb = imageio.imread(rgb_files[0])
        # resize the image to target size
        rgb = cv2.resize(rgb, dsize=(self.target_w, self.target_h), interpolation=cv2.INTER_AREA)
        rgb = rgb.astype(np.float32) / 255.0
