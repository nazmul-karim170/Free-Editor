...
class ClipSimilarity(nn.Module):
    def __init__(self, name: str = "ViT-L/14"):
        ...
        self.model, _ = clip.load(name, device='cuda' if torch.cuda.is_available() else 'cpu', download_root="./")
    ...
    def forward(
        self, image_0: torch.Tensor, image_1: torch.Tensor, text_0: list[str], text_1: list[str], image_target: torch.Tensor, image_start: torch.Tensor, text_caption: list[str]
        ...
        text_features_caption = self.encode_text(text_caption)
        image_features_target = self.encode_image(image_target)
        image_features_start = self.encode_image(image_start)  

        sim_direction_0 = F.cosine_similarity(image_features_target - text_features_0, text_features_caption - text_features_0)
        sim_direction_1 = F.cosine_similarity(image_features_start - text_features_1, text_features_caption - text_features_1)

        return sim_0, sim_1, sim_direction_0, sim_direction_1, sim_image
