...
class IBRNetCollectedGenerationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        nearest_pose_ids = get_nearest_pose_ids(
            render_pose,
            train_poses,
            min(self.num_source_views * subsample_factor+1, 22),
            tar_id=id_render,
            angular_dist_method="dist",
            scene_center=world_center,
        )
        nearest_pose_ids = np.random.choice(
            nearest_pose_ids, min(num_select+1, len(nearest_pose_ids)), replace=False
        )
        ...
        caption_rgb = imageio.imread(self.render_rgb_files[0]).astype(np.float32) / 255.0
        ...
        return {
            ...
            "num_images_in_scene": len(self.render_rgb_files),
