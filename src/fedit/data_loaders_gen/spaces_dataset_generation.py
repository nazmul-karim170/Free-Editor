...
class SpacesFreeGenerationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        nearby_view_ids_selected = np.random.choice(
            sorted_ids[1:], self.num_source_views+1, replace=False
        )
        ...
        starting_rgb = ref_rgbs_np[0]
        starting_camera = ref_cameras[0]
        src_rgbs     = np.stack(ref_rgbs_np[1:], axis=0)
        src_cameras  = np.stack(ref_cameras[1:], axis=0)
        nearest_pose_ids = nearby_view_ids_selected[1:]
