...
class NerfSynthGenerationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        nearest_pose_ids = get_nearest_pose_ids(
            render_pose,
            train_poses,
            int(self.num_source_views * 2+1),  #+int(max_dif/3)
            tar_id=id_render,
        ...
        return {
            ...
            "traget_rgb": torch.from_numpy(rgb[..., :3]),
            "traget_camera_matrices": torch.from_numpy(camera),
            "starting_view": torch.from_numpy(starting_rgb[..., :3]),
            "starting_camera_matrices": torch.from_numpy(starting_camera),
